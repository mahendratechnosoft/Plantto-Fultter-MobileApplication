...
abstract class AuthState with _$AuthState {
  const factory AuthState({
    ...
    @Default(false) bool canApproveAttendance,
  }) = _AuthState;
  ...
  Future<void> fetchPermission() async {
    try {
        ...
        if (res.statusCode == 200) {
          ...
          state = state.copyWith(
            ...
            canApproveAttendance:
                permissionModel.timeSheetAttendanceApproveAccess ?? false,
          );
        }
      }

      await ref.read(homeProvider.notifier).fetchDesignersV2();
      await ref.read(homeProvider.notifier).fetchWorkOrders();
      ...
      if (state.isAdmin || state.canAccessTimeSheet) {
        ...
        ref.read(timeSheetProvider.notifier).updateTimeSheetModel();
      }
    } catch (e) {
      rethrow;
    } finally {}
