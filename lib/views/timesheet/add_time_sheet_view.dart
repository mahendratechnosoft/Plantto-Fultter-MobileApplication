...
class _HomeViewState extends ConsumerState<AddTimeSheetView> {
  ...
  @override
  void dispose() {
    // ref.read(homeProvider.notifier).updateTaskSheetCreating(false);
    super.dispose();
  }

  Future<void> populateDataIfEditing() async {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      userIsAdmin = ref.watch(authProvider.select((value) => value.isAdmin));
      setState(() {});
    });
    ...
    if (model != null) {
      ...
      for (String workOrder in ref.read(homeProvider).workOrdersV2 ?? []) {
        {
      ...
      for (EmployeeList designer
          in ref.read(homeProvider).designersV2?.employeeList ?? []) {
      ...
      for (String process in ref.watch(
        homeProvider.select((value) => value.processNoV2 ?? []),
      )) {
      ...
      child: Consumer(
        builder: (context, ref, _) {
          final isApiCalling = ref.watch(
            homeProvider.select((value) => value.isTaskSheetCreating),
          );

          final isLoading = ref.watch(
            homeProvider.select((value) => value.isWorkOrderFetching),
          );
  ...
  Widget _buildItemNumberSelectionField(WidgetRef ref) {
    return CommonDropdown<String>(
      items: ref.watch(
        homeProvider.select((value) => value.workOrdersV2 ?? []),
      ),
      label: "Item Number",
      onChanged: (value) {
        try {
  ...
  WorkOrder? findWorkOrderByProcess(ProcessList targetProcess) {
    List<WorkOrder> workOrders = ref.watch(
      homeProvider.select((value) => value.workOrders ?? []),
    );
  ...
  Future<void> saveButtonTap() async {
    ...
    if (validationKey.currentState?.validate() ?? false) {
      ...
      final timeSheetNotifier = ref.read(timeSheetProvider.notifier);
      try {
