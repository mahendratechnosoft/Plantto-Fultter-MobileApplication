...
class _TimeSheetListingState extends ConsumerState<HomeView> {
  ...
  Future<void> initCall() async {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      ref.read(homeProvider.notifier).updateWorkOrderFetching(true);
      try {
        await authNotifier.fetchPermission();
      } catch (e) {
        showAlert(context: context, message: e);
      } finally {
        ref.read(homeProvider.notifier).updateWorkOrderFetching(false);
      }
    });
  }
          ...
          Consumer(
            builder: (context, ref, _) {
              ...
              bool canApproveAttendance =
                  authState.canApproveAttendance || authState.isAdmin;
              return canApproveAttendance
                  ? SizedBox(
                      width: 200,
                      child: CommonElevatedButton(
                        onTap: () {
                          context.pushNamed(AppRoutes.timeSheetApproval);
                        },
                        text: CustomText(
                          txtTitle: "Approve Attendance",
            ...
            onPressed: () {
              showDeleteConfirmationDialog(
                ...
                onDelete: () async {
                  ...
                  try {
                    await ref.read(authProvider.notifier).logOutApiCall();
        ...
        child: Consumer(
          builder: (context, ref, _) {
            bool isApiCalling = ref.watch(
              timeSheetProvider.select((value) => value.isTimeSheetFetching),
            );

            List<TimeSheetModel> allTimeSheets = List.from(
              ref.watch(
                homeProvider.select((value) => value.filteredTimeSheet ?? []),
              ),
            );
            var filterState = ref.watch(timeSheetFilterProvider);
            ...
            if (ref.watch(
              homeProvider.select((value) => value.isAnyFieldSelectedForFilter),
            )) {
              allTimeSheets.sort(
                (a, b) => DateTime.parse(
                  a.date ?? "",
            ...
            if (isApiCalling && allTimeSheets.isEmpty) {
              ...
              return RefreshIndicator(
                onRefresh: () async {
                  try {
                    await ref.read(authProvider.notifier).fetchPermission();
                  } catch (e) {
                    showAlert(context: context, message: e);
                  }
                },
